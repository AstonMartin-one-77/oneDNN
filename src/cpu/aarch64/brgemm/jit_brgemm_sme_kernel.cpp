...
#define GET_OFF_BATCH_ELEMENT(field) (uint32_t)offsetof(brgemm_batch_element_t, field)
...
void jit_brgemm_sme_kernel_base_t::generate() {
    const int sme_capacity = get_sme_length() >> 2u; // float == 4 bytes per sample
     ...
     */
    auto ld1w_A_to_za0 = [=](const WReg &zaRegOffset, const _PReg &pA/*div T_z*/, const XReg &A_ptr, int bd) {
        int step_offset = 0; // used to increase base offset of ZA tile to access lines > 15 (for ld1w instruction)
    ...
    auto fmopa_A_T_by_B = [=](const std::vector<ZReg> &A_zregs, const std::vector<ZReg> &B_zregs, 
                              const _PReg &pA_T/*div T_m*/, const _PReg &pB/*div T_m*/, const _PReg &pB_tail/*div T_m*/, 
                              const WReg &zaRegOffset, const XReg &B_ptr, int rd, int ld) {
        int step_offset = 0; // used to increase base offset of ZA tile to access lines > 3 (for mova instruction)
        int cycles = (rd > sme_capacity) ? sme_capacity : rd;
        int used_tiles = std::min((ld / sme_capacity), 3); // Up to 3 tiles can be used (4th is used for A)
    ...
    auto st1w_tiles_to_D = [=](const std::vector<ZReg> &D_zregs, const PReg &pD/*==pB*/, const PReg &pD_tail/*==pB_tail*/, 
                               const WReg &zaRegOffset, const XReg &D_ptr, int stride, int bd, int ld) {
        int step_offset = 0; // used to increase base offset of ZA tile to access lines > 3 (for mova instruction)
    ...
    int bdb = brg.bcast_dim / bd_block; // Number of blocks along broadcast dim of A
    ...
    std::vector<ZReg> B_zregs = {z0,z1,z2,z3,z4,z5,z6,z7,z8,z9,z10,z11,z12,z13,z14,z15,z16,z17,z18,z19,z20,z21,z22,z23};
    ...
    printf("SME:LDA=%d, bd=%d, LDB=%d, ld=%d, LDD=%d, ld=%d\r\n", brg.LDA, brg.bcast_dim, brg.LDB, brg.load_dim, brg.LDD, brg.load_dim);
    auto rd_loop = [=](p_block_desc_t &bd, p_block_desc_t &ld) {
        ...
        if (rdb > 0) {
            ...
            fmopa_A_T_by_B(A_zregs, B_zregs, bd.pReg/T_m, p0/T_m, p2/T_m, w13, x12,
                           rd_block, ld.size);
            if (rdb > 1) {
                subs(w22, w22, 1);
                b(GT, rdb_loop);
    ...
    ptrue(p0.s);
    mov(x20, bd_tail);
    mov(x21, ld_tail % sme_capacity); // used up to 3 Tiles => tail predicate for last Tile only
    mov(x22, rd_tail);
